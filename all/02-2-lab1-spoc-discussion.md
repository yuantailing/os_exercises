...
> 以下程序中，sum函数的功能：计算一系列未知个数的非零整数的和，参数以0结束。
...
> 先调好一个中断源（例如键盘）和一个外设（例如串口）。这时，中断控制器比较简单，中断向量表也不需要，因为一定只对应键盘中断的中断服务程序。

> 再加入其它输出外设（例如CGA等）。这一步，中断控制器和中断向量表都不需要调整。

> 然后加入其它中断源（例如时钟中断等）。这一步，需要丰富初始化8259中断控制器和IDT的代码。

> 最后加入加载ELF格式文件的功能，就能得到一个与Lab1功能近似的操作系统。可用显示字符串来测试一下。
